# Hexadecimal to binary conversion (now supports all characters)
def text2bin(s):
    bin_str = ''
    for char in s:
        # Convert each character to its ASCII equivalent and then to binary
...
def bin2text(bin_str):
    ...
    for i in range(0, len(bin_str), 8):
        byte = bin_str[i:i+8]
        # Convert each 8-bit segment back to an ASCII character
        text += chr(int(byte, 2))
    return text
...
# Permutation made after each S-box substitution
per = [16, 7, 20, 21,
       29, 12, 28, 17,
       1, 15, 23, 26,
       5, 18, 31, 10,
...
def encrypt(pt, rkb, rk):
    pt = text2bin(pt)  # Convert plain text to binary
...
if __name__ == "__main__":
    ...
    print("Cipher Text (in binary):", cipher_text)

    print("Decryption")
    rkb_rev = rkb[::-1]
    rk_rev = rk[::-1]
    decrypted_bin = encrypt(bin2text(cipher_text), rkb_rev, rk_rev)
    print("Decrypted Plain Text:", bin2text(decrypted_bin))
